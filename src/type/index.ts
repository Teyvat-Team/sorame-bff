...
type UnPromisifyFunction<T> = T extends PromiseFunctionType<infer U>
  ? U
  : never;
...
type ObjectAndArrayPropKeys<T> = {
  [K in keyof T]: T[K] extends any[] | object ? K : never;
}[keyof T];
...
type NonObjectAndArrayPicks<T> = Pick<
  T,
  Exclude<keyof T, ObjectAndArrayPropKeys<T>>
>;

type ObjectAndArrayPicks<T> = Pick<
  T,
  Exclude<keyof T, keyof NonObjectAndArrayPicks<T>>
>;
...
type CascadeSelectPropsKey<T> = {
  default: T extends object
    ? Obj<T[keyof T]> extends object
      ?
          | NonObjectAndArrayPicks<T>
          | CascadeSelectPropsKey<
              ObjectAndArrayPicks<T>[keyof ObjectAndArrayPicks<T>]
            >
      : T
    ...
    ?
        ...
        | CascadeSelectPropsKey<
            ObjectAndArrayPicks<UnArray<T>>[keyof ObjectAndArrayPicks<
              UnArray<T>
            >]
          >
    : T;
...
type FlattenCascaded<T> = UnionToIntersection<
  | T
  | NonObjectAndArrayPicks<T>
  | CascadeSelectPropsKey<ObjectAndArrayPicks<T>[keyof ObjectAndArrayPicks<T>]>
>;
